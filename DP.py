...
from pyqt_items.pyqt_CursorItem import CursorItem
from pyqt_items.pyqt_ZoomItem import ZoomItem
from pyqt_items.pyqt_PlotWidgetKP import PlotWidgetKP
from pyqt_items.pyqt_ResizeItem import ResizeItem
from pyqt_items.pyqt_CircularListWidget import CircularListWidget
...
class GUI_DisplayPanel(object):
    ...
    def mysetup(self):
        ...
        self.key2=QtCore.Qt.Key_Shift # (For some reason the shift key does not auto-repeat when pressed)
        self.key3=QtCore.Qt.Key_Y
        self.key4=QtCore.Qt.Key_X
        self.groupkey=QtCore.Qt.Key_C

        # Show/hide cursor key
        self.shkeys=[QtCore.Qt.Key_1,QtCore.Qt.Key_2,QtCore.Qt.Key_3,QtCore.Qt.Key_4]

        # Switch crosshair state key
        self.switchkeys=[QtCore.Qt.Key_Q,QtCore.Qt.Key_W,QtCore.Qt.Key_E,QtCore.Qt.Key_R]

        # Show/hide DCs key
        self.DCkeys=[QtCore.Qt.Key_A,QtCore.Qt.Key_S,QtCore.Qt.Key_D,QtCore.Qt.Key_F]

        # Switch integration-crosshair state key
        self.iswitchkeys=[QtCore.Qt.Key_5,QtCore.Qt.Key_6,QtCore.Qt.Key_7,QtCore.Qt.Key_8]

        # Switch integration-crosshair fade key
        self.fadekey=QtCore.Qt.Key_G

        # Hide/show all active objects key
        self.allkey=QtCore.Qt.Key_Space

        # Reset cursors key
        self.resetkey=QtCore.Qt.Key_Return
    ...
    def GUI_setup(self):
        ...
        p.setColor(self.centralwidget.backgroundRole(), QtCore.Qt.white) # Input color here
        ...
        self.Cmaps.addItems(["Greys","Blues","bone","spring","summer","autumn","winter","cool","jet","ocean"]) # Choose your favourites
        ...
        self.SpanX.setLocale(QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.UnitedStates)) # Decimal separator '.'
        ...
        self.SpanY.setLocale(QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.UnitedStates)) # Decimal separator '.'
    ...
    def GUI_files(self,files):
        ...
        self.GUI_setdata(**self.xarray2dict(self.files[0]))
        ...
        def select_file():
            ...
            self.GUI_setdata(**self.xarray2dict(self.files[self.Files.currentRow()]))
    ...
    def GUI_auxiliary(self):
        ...
        self.lineupv_sig=partial(self.lineupv,QtCore.Qt.Key_V) # Signal object 44

        # Signals to line up the cursors horizontally
        self.lineuph_sig=partial(self.lineuph,QtCore.Qt.Key_H) # Signal object 45
    ...
    def arrowmove(self,csr_num,evt):
        ...
        if self.cursors[csr_num-1] in self.MainPlot.getViewBox().allChildren():
            ...
            if evt.key() == QtCore.Qt.Key_Up and self.cursors[csr_num-1].data['pos'][0][1] + self.ispany < self.y_max:
            ...
            if evt.key() == QtCore.Qt.Key_Down and self.cursors[csr_num-1].data['pos'][0][1] - self.ispany > self.y_min:
                self.cursors[csr_num-1].setData(pos=np.array([[self.cursors[csr_num-1].data['pos'][0][0],self.y[self.find_nearest(self.y,self.cursors[csr_num-1].data['pos'][0][1])-1]]]), **self.dicts[csr_num-1])

            # Move cursor data-column right
            if evt.key() == QtCore.Qt.Key_Right and self.cursors[csr_num-1].data['pos'][0][0] + self.ispanx < self.x_max:
            ...
            if evt.key() == QtCore.Qt.Key_Left and self.cursors[csr_num-1].data['pos'][0][0] - self.ispanx > self.x_min:
                self.cursors[csr_num-1].setData(pos=np.array([[self.x[self.find_nearest(self.x,self.cursors[csr_num-1].data['pos'][0][0])-1],self.cursors[csr_num-1].data['pos'][0][1]]]), **self.dicts[csr_num-1])
    ...
    # Convert 2D xarray to dictionary
    def xarray2dict(self,A):
        ...
        return  {'x':x,'y':y,'z':z,'xdim':xdim,'ydim':ydim,'zdim':zdim,'xuts':xuts,'yuts':yuts,'zuts':zuts}
